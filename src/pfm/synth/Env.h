...
private:
    ...
    float stateTarget[ENV_NUMBER_OF_STATES] = {0};
    // float

    float stateInc[ENV_NUMBER_OF_STATES] = {0};
    uint8_t nextState[ENV_NUMBER_OF_STATES] = {0};
    struct table tables[ENV_NUMBER_OF_STATES] = {{0}};

    EnvelopeParamsA* envParamsA = nullptr;
    EnvelopeParamsB* envParamsB = nullptr;
    uint8_t envNumber = 0;
    float* algoNumber = nullptr;
