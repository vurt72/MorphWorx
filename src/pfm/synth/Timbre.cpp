...
// Regular memory
float midiNoteScale[2][NUMBER_OF_TIMBRES][128] = {{{0}}};
...
float sat25(float x)
{
	// Branchless version — avoids branch misprediction in filter inner loops
	float ax = fabsf(x * 0.25f);
	float g = 1.0f - ax;
	g = fmaxf(0.0f, g);  // clamp to [0,1] — compiles to VMAXNM on A7
	return x * g;
}
inline
float sat66(float x)
...
float cosf_fast(float x) {
	// 4th-order Taylor: cos(x) ≈ 1 - x²/2 + x⁴/24
	// Max error ~0.0002 for |x| < π — fine for per-block filter coefficients
	float x2 = x * x;
	return 1.0f - x2 * (0.5f - x2 * 0.041666667f);
}
...
case FILTER_SVFLH:
{
	...
	float arghp = (1 + 2 * cos_theta);
	float p = arghp - sqrtf(arghp * arghp - 1);
	//0 < theta < Pi/2
...
void Timbre::midiClockContinue(int songPosition) {
    ...
    for (int k = 0; k < params.engine1.numberOfVoice; k++) {
		int vn = (int)voiceNumber[k];
		voices[vn]->midiClockContinue(songPosition);
    }
...
void Timbre::midiClockSongPositionStep(int songPosition) {
    ...
    for (int k = 0; k < params.engine1.numberOfVoice; k++) {
		int vn = (int)voiceNumber[k];
		voices[vn]->midiClockSongPositionStep(songPosition,  this->recomputeNext);
    }
...
void Timbre::resetMatrixDestination(float oldValue) {
    for (int k = 0; k < params.engine1.numberOfVoice; k++) {
		int vn = (int)voiceNumber[k];
		voices[vn]->matrix.resetDestination(oldValue);
    }
}
...
void Timbre::setMatrixSource(enum SourceEnum source, float newValue) {
    for (int k = 0; k < params.engine1.numberOfVoice; k++) {
		int vn = (int)voiceNumber[k];
		voices[vn]->matrix.setSource(source, newValue);
    }
}
