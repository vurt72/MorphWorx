...
private:
    ...
    float sampleBlock[BLOCK_SIZE * 2] = {0};
    float *sbMax = nullptr;
    float numberOfVoiceInverse = 1.0f;
    float mixerGain = 1.0f;
    Voice *voices[MAX_NUMBER_OF_VOICES] = {nullptr};
    bool holdPedal = false;
    uint8_t lastPlayedVoiceNum = 0;
#ifdef CVIN
    float cvFrequency = 440.0f;
#endif
    ...
    float ticksEveryNCalls = 0.0f;
    int ticksEveyNCallsInteger = 0;



    float arpegiatorStep = 0.0f;
    ...
    int8_t current_octave_ = 0;
    int8_t current_step_ = 0;
	int8_t start_step_ = 0;
    uint8_t ignore_note_off_messages_ = 0;
    uint8_t recording_ = 0;
    // Low pass filter
    float fxParam1 = 0.0f, fxParam2 = 0.0f, fxParam3 = 0.0f;
    float v0L = 0, v1L = 0, v2L = 0, v3L = 0, v4L = 0, v5L = 0, v6L = 0, v7L = 0, v8L = 0;
    float v0R = 0, v1R = 0, v2R = 0, v3R = 0, v4R = 0, v5R = 0, v6R = 0, v7R = 0, v8R = 0;
   	float fxParamA1 = 0, fxParamA2 = 0, fxParamB2 = 0;
    // save float fxParam1 to detect modification
    float fxParam1PlusMatrix = -1.0f;
    float lastVelocity = 0.0f;
    // lfoUsed
    uint8_t lfoUSed[NUMBER_OF_LFO] = {0};
    // seqStartUsed
    uint8_t seqStartUsed[NUMBER_OF_LFO_STEP] = {0};
    // Left right balance
    float leftRightBalance = 0.5f;
