...
float Voice::glidePhaseInc[nbGlideVals] = {0};

extern float *frequencyToUse;

Voice::Voice(void)
{
	// Zero-initialize all POD struct members (OscState, EnvData)
	// On bare-metal ARM (MetaModule), new does NOT zero-initialize memory.
	memset(&envState1, 0, sizeof(EnvData));
	memset(&envState2, 0, sizeof(EnvData));
	memset(&envState3, 0, sizeof(EnvData));
	memset(&envState4, 0, sizeof(EnvData));
	memset(&envState5, 0, sizeof(EnvData));
	memset(&envState6, 0, sizeof(EnvData));
	memset(&oscState1, 0, sizeof(OscState));
	memset(&oscState2, 0, sizeof(OscState));
	memset(&oscState3, 0, sizeof(OscState));
	memset(&oscState4, 0, sizeof(OscState));
	memset(&oscState5, 0, sizeof(OscState));
	memset(&oscState6, 0, sizeof(OscState));

	// Initialize all scalar members to safe defaults
	frequency = 0;
	released = false;
	playing = false;
	index = 0;
	note = 0;
	velocity = 0.0f;
	velIm1 = velIm2 = velIm3 = velIm4 = velIm5 = 0.0f;
	newNotePlayed = false;
	freqAi = freqAo = 0.0f;
	freqBi = freqBo = 0.0f;
	freqHarm = 1.0f;
	...
	glidePhase = 0.0f;
	nextGlidingNote = 0;
	noteFrequencyUnison = 1.0f;
	phase = 0.25f;
	env1ValueMem = env2ValueMem = env3ValueMem = 0.0f;
	env4ValueMem = env5ValueMem = env6ValueMem = 0.0f;
	currentTimbre = nullptr;
	modulationIndex1 = modulationIndex2 = modulationIndex3 = 0.0f;
	modulationIndex4 = modulationIndex5 = 0.0f;
	mix1 = mix2 = mix3 = mix4 = mix5 = mix6 = 0.0f;
	pan1Left = pan2Left = pan3Left = pan4Left = pan5Left = pan6Left = 0.0f;
	pan1Right = pan2Right = pan3Right = pan4Right = pan5Right = pan6Right = 0.0f;
	noteAlreadyFinished = 0;
