...
public:
    ...
    inline void calculateFrequencyWithMatrix(struct OscState *oscState, Matrix* matrix, float expHarm) {
        // Cache WaveTable pointer once per block to avoid repeated pointer-chase
        // in getNextSample() (oscillator->shape doesn't change within a block)
        cachedWT = &waveTables[(int) oscillator->shape];
    ...
    inline float getNextSample(struct OscState *oscState)  {
        // Uses cachedWT set by calculateFrequencyWithMatrix() — avoids
        // re-deriving &waveTables[(int)oscillator->shape] every sample
        oscState->index +=  oscState->frequency * cachedWT->precomputedValue + cachedWT->floatToAdd;
        ...
        indexInteger &= cachedWT->max;
        // Readjust the floating pont inside the table
        oscState->index += indexInteger;

        return cachedWT->table[indexInteger];
    }
   	...
   	float* getNextBlockHQ(struct OscState *oscState)  {
    	...
    	oscValuesCpt &= 0x3;
   		for (int k=0; k<32; ) {
            ...
            iIndex &=  max;
            fIndex += iIndex;
            oscValuesToFill[k++] = wave[iIndex] * (1-fp) + wave[iIndex + 1] * fp;
...
private:
    ...
    DestinationEnum destFreq = (DestinationEnum)0;
    Matrix* matrix = nullptr;
    static float* oscValues[4];
    static int oscValuesCpt;
    OscillatorParams* oscillator = nullptr;
    WaveTable* cachedWT = nullptr; // Cached per-block by calculateFrequencyWithMatrix()
};
