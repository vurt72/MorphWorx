...
Place `.wav` files in `sdc:/phaseon1/` on your SD card. In the module, use the browse controls to select a file.
...
The factory bank bundled inside the plugin package is loaded automatically on first use and serves as the fallback if the user bank is missing.
...
2. Download `MorphWorx-phaseon1-sdcard.zip` from the Releases page, extract it, and copy the `phaseon1/` folder to the **root of your SD card**:
   - `sdc:/phaseon1/Phbank.bnk` — factory preset bank (loads automatically)
   - `sdc:/phaseon1/phaseon1.wav` — default wavetable (loads automatically)
